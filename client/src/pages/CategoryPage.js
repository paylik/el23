  ...
  const onSortChange = (event) => {
    ...
    const addToCart = (product, ref) => {
     if (auth.token) {
       ...
       ref.current.show({ severity: 'info', summary: 'Товар добавлен в корзину',
         detail: 'Товар добавлен в корзину', life: 3000 })
     }
     else {
       ref.current.show({ severity: 'error', summary: 'Товар могут добавлять только зарегистрированные пользователи',
         detail: 'Товар могут добавлять только зарегистрированные пользователи', life: 3000 })
     }
    }

  const header = () => {
    return <Dropdown options={sortOptions} value={sortKey} optionLabel="label" placeholder="Сортировать" onChange={onSortChange} className="w-full sm:w-14rem" />
  }
    ...
    return (
      <div className="col-12">
        <div className="flex flex-column xl:flex-row xl:align-items-start p-4 gap-4">
          <img className="w-5 sm:w-10rem xl:w-7rem shadow-2 block xl:block mx-auto border-round" src={img} alt={product.name} />
          <div className="flex flex-column sm:flex-row justify-content-between align-items-center xl:align-items-start flex-1 gap-4">
            <div className="flex flex-column align-items-center sm:align-items-start gap-3" onClick={() => products.setProduct(product)}>
              <Link to={`/product/${product._id}`}>
  ...
  return (
    <div>
      ...
      <div className="w-10 m-auto">
        ...
        <DataView value={products.categoryProducts} itemTemplate={itemTemplate} header={header()} sortField={sortField} sortOrder={sortOrder} />
      </div>
    </div>
  )
}
